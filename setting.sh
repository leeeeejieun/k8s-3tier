...
do
    if [ $ip != '192.168.10.10' ]; then
        sshpass -p 'centos' scp -o StrictHostKeyChecking=no /etc/hosts root@$ip:/etc/hosts
    fi
done
...
# 5. wheel 그룹에 속한 사용자가 암호 입력 없이 sudo 명령어를 사용할수 있도록 설정하기
for ip in $IP_LIST
do
...
for ip in $IP_LIST
do
    ssh $ip "sudo systemctl disable --now firewalld"
done

# 7. Multi-user.target으로 전환
for ip in ${TARGET_IPS[@]}
do
    ssh $ip "sudo systemctl set-default multi-user.target && sudo systemctl isolate multi-user.target"
done

# 8. 바탕화면 아이콘 생성 - 바탕화면에 생성된 아이콘을 확인하고 실행 허용(Allow Launching) 선택(개별적으로 진행)
for ip in $IP_LIST; 
do
